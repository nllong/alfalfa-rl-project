...
if len(sys.argv) <= 2:
    raise Exception('Expecting arguments of <model_name> and <mo_path>')

print(sys.argv)
MO_PATH = sys.argv[1]  # Relateive path to the MO file (e.g., SingleZoneVAV.mo)
MODEL_NAME = sys.argv[2]  # Model to build (e.g., SingleZoneVAV.TestCaseSupervisory)
...
def parse_instances(model_path, file_name):
    ...
    # Get all parameters
    allvars = fmu.get_model_variables(variability=0).keys() + fmu.get_model_variables(variability=1).keys()
...
def write_wrapper(model_path, file_name, instances):
    ...
    with open(wrapped_path, 'w') as f:
        ...
        for block in instances['Overwrite'].keys():
            ...
            input_signals_w_info[block] = _make_var_name(block, style='input_signal',
            ...
            input_activate_w_info[block] = _make_var_name(block, style='input_activate',
        ...
        for block in instances['Read'].keys():
            ...
            f.write('\tModelica.Blocks.Interfaces.RealOutput {0} = mod.{1}.y "{2}";\n'.format(
                _make_var_name(block, style='output',
                               attribute='(unit="{0}")'.format(instances['Read'][block]['Unit'])), block,
                instances['Read'][block]['Description']))
        # Add original model
        f.write('\t// Original model\n')
        f.write('\t{0} mod(\n'.format(model_path))
        # Connect inputs to original model overwrite and activate signals
        for i, block in enumerate(instances['Overwrite']):
            f.write('\t\t{0}(uExt(y={1}),activate(y={2}))'.format(block, input_signals_wo_info[block],
...
# fmu_path, kpi_path = export_fmu(args.model_name, [args.path])

# model_path = 'SingleZoneVAV.TestCaseSupervisory'
# mo_path = 'SingleZoneVAV.mo'
# fmu_path, kpi_path = export_fmu(model_path, [mo_path])
fmu_path, kpi_path = export_fmu(MODEL_NAME, [MO_PATH])
# Print information
