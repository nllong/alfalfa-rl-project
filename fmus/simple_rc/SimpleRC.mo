...
model SimpleRC
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor res(
    R=0.01)
  ...
  Buildings.Utilities.IO.SignalExchange.Overwrite oveSet(
    u(
      unit="K",
      min=273.15+10,
      max=273.15+35),
  ...
  Buildings.Utilities.IO.SignalExchange.Overwrite oveAct(
    u(
      unit="W",
      min=0,
      max=3000),
  ...
  Buildings.Utilities.IO.SignalExchange.Read TZone(
    KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.AirZoneTemperature,
    y(
      unit="K"),
    description="Zone temperature")
    annotation (Placement(transformation(extent={{100,10},{120,30}})));
  Buildings.Utilities.IO.SignalExchange.Read PHeat(
  ...
  Buildings.Utilities.IO.SignalExchange.Read setZone(
    y(
      unit="K"),
    description="Zone temperature setpoint")
...
equation
  ...
  connect(senTZone.T,con.u_m)
    annotation (Line(points={{70,20},{80,20},{80,-40},{-30,-40},{-30,-32}},color={0,0,127}));
  ...
  connect(oveAct.y,eff.u)
    annotation (Line(points={{11,-20},{14,-20},{14,-80},{18,-80}},color={0,0,127}));
  connect(senTZone.T,TZone.u)
    annotation (Line(points={{70,20},{98,20}},color={0,0,127}));
  connect(eff.y,PHeat.u)
    annotation (Line(points={{41,-80},{98,-80}},color={0,0,127}));
  connect(oveSet.y,setZone.u)
    annotation (Line(points={{-49,-20},{-46,-20},{-46,-60},{-42,-60}},color={0,0,127}));
  annotation (
    uses(
      Modelica(
        version="3.2.2"),
