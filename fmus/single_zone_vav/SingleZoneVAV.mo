...
package SingleZoneVAV
  ...
  model TestCaseSupervisory
    ...
    Buildings.Utilities.IO.SignalExchange.Overwrite oveTSetRooHea(
      u(
        unit="K",
        min=273.15+10,
        max=273.15+35),
    ...
    Buildings.Utilities.IO.SignalExchange.Overwrite oveTSetRooCoo(
      u(
        unit="K",
        min=273.15+10,
        max=273.15+35),
    ...
    Buildings.Utilities.IO.SignalExchange.Read PPum(
      y(
        unit="W"),
    ...
    Buildings.Utilities.IO.SignalExchange.Read PFan(
      y(
        unit="W"),
      KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.ElectricPower,
      description="Fan electrical power")
      annotation (Placement(transformation(extent={{140,130},{160,150}})));
    Buildings.Utilities.IO.SignalExchange.Read TRooAir(
      y(
        unit="K"),
    ...
    Buildings.Utilities.IO.SignalExchange.Read senTSetRooCoo(
      ...
      annotation (Placement(transformation(extent={{-10,-10},{10,10}},rotation=180,origin={-142,-36})));
    Buildings.Utilities.IO.SignalExchange.Read senTSetRooHea(
      y(
        unit="K"),
      KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.None,
      description="Room heating setpoint")
      annotation (Placement(transformation(extent={{-10,-10},{10,10}},rotation=180,origin={-142,64})));
    ...
    Buildings.Utilities.IO.SignalExchange.Overwrite oveUSetFan(
      u(
        unit="Fraction",
        min=0,
        max=10),
      description="Override Fan Control")
  ...
  equation
    connect(weaDat.weaBus,weaBus)
      annotation (Line(points={{-140,130},{-124,130}},color={255,204,51},thickness=0.5),Text(string="%second",index=1,extent={{6,3},{6,3}}));
    connect(con.yHea,hvac.uHea)
      annotation (Line(points={{-91,6},{-56,6},{-56,12},{-40,12}},color={0,0,127}));
    connect(con.yCooCoiVal,hvac.uCooVal)
      annotation (Line(points={{-91,0},{-54,0},{-54,5},{-40,5}},color={0,0,127}));
    connect(con.yOutAirFra,hvac.uEco)
      annotation (Line(points={{-91,3},{-50,3},{-50,-2},{-40,-2}},color={0,0,127}));
    connect(hvac.chiOn,con.chiOn)
      annotation (Line(points={{-40,-10},{-60,-10},{-60,-4},{-91,-4}},color={255,0,255}));
    connect(con.TSetSupChi,hvac.TSetChi)
      annotation (Line(points={{-91,-8},{-70,-8},{-70,-15},{-40,-15}},color={0,0,127}));
    connect(con.TMix,hvac.TMix)
      annotation (Line(points={{-114,2},{-122,2},{-122,-40},{10,-40},{10,-4},{3,-4}},color={0,0,127}));
    connect(hvac.supplyAir,zon.supplyAir)
      annotation (Line(points={{2,8},{10,8},{10,2},{40,2}},color={0,127,255}));
    connect(hvac.returnAir,zon.returnAir)
      annotation (Line(points={{2,0},{6,0},{6,-2},{40,-2}},color={0,127,255}));
    connect(con.TOut,weaBus.TDryBul)
      annotation (Line(points={{-114,-2},{-124,-2},{-124,130}},color={0,0,127}));
    connect(hvac.weaBus,weaBus)
      annotation (Line(points={{-34,17.8},{-34,130},{-124,130}},color={255,204,51},thickness=0.5));
    connect(zon.weaBus,weaBus)
      annotation (Line(points={{46,18},{42,18},{42,130},{-124,130}},color={255,204,51},thickness=0.5));
    connect(con.TSup,hvac.TSup)
      annotation (Line(points={{-114,-9},{-118,-9},{-118,-32},{4,-32},{4,-8},{3,-8}},color={0,0,127}));
    connect(con.TRoo,zon.TRooAir)
      annotation (Line(points={{-114,-6},{-120,-6},{-120,-36},{6,-36},{6,-22},{90,-22},{90,0},{81,0}},color={0,0,127}));
    connect(hvac.PFan,EFan.u)
      annotation (Line(points={{3,18},{24,18},{24,-40},{38,-40}},color={0,0,127}));
    connect(hvac.QHea_flow,EHea.u)
      annotation (Line(points={{3,16},{22,16},{22,-70},{38,-70}},color={0,0,127}));
    connect(hvac.PCoo,ECoo.u)
      annotation (Line(points={{3,14},{20,14},{20,-100},{38,-100}},color={0,0,127}));
    connect(hvac.PPum,EPum.u)
      annotation (Line(points={{3,12},{18,12},{18,-130},{38,-130}},color={0,0,127}));
    connect(EFan.y,EHVAC.u[1])
      annotation (Line(points={{61,-40},{70,-40},{70,-54.75},{80,-54.75}},color={0,0,127}));
    connect(EHea.y,EHVAC.u[2])
      annotation (Line(points={{61,-70},{64,-70},{64,-60},{66,-60},{66,-60},{80,-60},{80,-58.25}},color={0,0,127}));
    connect(ECoo.y,EHVAC.u[3])
      annotation (Line(points={{61,-100},{70,-100},{70,-61.75},{80,-61.75}},color={0,0,127}));
    connect(EPum.y,EHVAC.u[4])
      annotation (Line(points={{61,-130},{74,-130},{74,-65.25},{80,-65.25}},color={0,0,127}));
    connect(TSetRooHea.y[1],oveTSetRooHea.u)
      annotation (Line(points={{-159,30},{-154,30}},color={0,0,127}));
    connect(oveTSetRooHea.y,con.TSetRooHea)
      annotation (Line(points={{-131,30},{-126,30},{-126,10},{-114,10}},color={0,0,127}));
    connect(TSetRooCoo.y[1],oveTSetRooCoo.u)
      annotation (Line(points={{-159,-10},{-154,-10}},color={0,0,127}));
    connect(oveTSetRooCoo.y,con.TSetRooCoo)
      annotation (Line(points={{-131,-10},{-126,-10},{-126,6},{-114,6}},color={0,0,127}));
    connect(hvac.PPum,PPum.u)
      annotation (Line(points={{3,12},{18,12},{18,80},{118,80}},color={0,0,127}));
    connect(hvac.PCoo,PCoo.u)
      annotation (Line(points={{3,14},{14,14},{14,100},{138,100}},color={0,0,127}));
    connect(hvac.QHea_flow,PHea.u)
      annotation (Line(points={{3,16},{10,16},{10,120},{118,120}},color={0,0,127}));
    connect(hvac.PFan,PFan.u)
      annotation (Line(points={{3,18},{6,18},{6,140},{138,140}},color={0,0,127}));
    connect(zon.TRooAir,TRooAir.u)
      annotation (Line(points={{81,0},{118,0}},color={0,0,127}));
    connect(oveTSetRooCoo.y,senTSetRooCoo.u)
      annotation (Line(points={{-131,-10},{-126,-10},{-126,-36},{-130,-36}},color={0,0,127}));
    connect(oveTSetRooHea.y,senTSetRooHea.u)
      annotation (Line(points={{-131,30},{-126,30},{-126,64},{-130,64}},color={0,0,127}));
    connect(EHVAC.y,ECumuHVAC.u)
      annotation (Line(points={{101.7,-60},{118,-60}},color={0,0,127}));
    connect(TOutdoorDB.u,weaBus.TDryBul)
      annotation (Line(points={{118,38},{-124,38},{-124,130}},color={0,0,127}),Text(string="%second",index=1,extent={{-6,3},{-6,3}},horizontalAlignment=TextAlignment.Right));
    connect(con.yFan,oveUSetFan.u)
      annotation (Line(points={{-91,9},{-91,18.5},{-72,18.5},{-72,62}},color={0,0,127}));
    connect(oveUSetFan.y,hvac.uFan)
      annotation (Line(points={{-49,62},{-46,62},{-46,18},{-40,18}},color={0,0,127}));
    connect(senUSetFan.u,hvac.uFan)
      annotation (Line(points={{-32,52},{-40,52},{-40,28},{-46,28},{-46,18},{-40,18}},color={0,0,127}));
    connect(zon.TRooRad,TRooRad.u)
      annotation (Line(points={{81,-3.4},{99.5,-3.4},{99.5,-28},{118,-28}},color={0,0,127}));
    annotation (
      experiment(
        StopTime=504800,
        Interval=3600,
        Tolerance=1e-06),
      __Dymola_Commands(
        file="modelica://Buildings/Resources/Scripts/Dymola/Air/Systems/SingleZone/VAV/Examples/ChillerDXHeatingEconomizer.mos" "Simulate and plot"),
      Documentation(
        info="<html>
<p>
...
First implementation.
  ...
  model Room
    ...
    Modelica.Blocks.Interfaces.RealOutput TRooRad
      "Room rad temperature"
      annotation (Placement(transformation(extent={{200,-40},{220,-20}}),iconTransformation(extent={{200,-44},{220,-24}})));
  protected
  ...
  equation
    ...
    connect(roo.heaPorRad,senTZonRad.port)
      annotation (Line(points={{58.7,-4.94},{111.35,-4.94},{111.35,-30},{162,-30}},color={191,0,0}));
