...
class Data_Manager(object):
    ...
    def __init__(self, testcase=None):
        ...
        # Find path to data directory - orig version had data directory
                    ...
                    for key in keys.drop('time'):
                        if key in appended and appended[key] is not None:
                            raise ReferenceError(
                                '{0} in multiple files within the Resources folder. These are: {1}, and {2}'.format(
                                    key, appended[key], f)
                            )
    ...
    def save_data_and_kpisjson(self, fmu_path):
        ...
        print("Saving the KPIs to the FMU")
    ...
    def load_data_and_kpisjson(self):
        ...
        self.case.data = pd.DataFrame(index=index, columns=all_keys).rename_axis('time')
        ...
        for f in files:
            ...
            if 'time' in keys:
                for key in keys.drop('time'):
                    for category in self.categories:
                        # Use linear interpolation for continuous variables
                        if key in self.categories[category] and category == 'weather':
                            g = interpolate.interp1d(df['time'], df[key], kind='linear')
...
if __name__ == "__main__":
    ...
    case_dir = os.path.join(os.path.split(os.path.split(os.path.abspath(__file__))[0])[0], 'testcase2')
